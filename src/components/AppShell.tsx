// App shell per the CGOPS Platform Design System: minimal header (logo, app
// name, brand indicator, user menu — no primary navigation in the header),
...
const NAV_PREF_KEY = 'nrc.nav.expanded'
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex h-14 items-center justify-between border-b border-surface-line bg-surface px-4 sm:px-6">
          <div className="flex items-baseline gap-2">
            <span className="text-sm font-semibold tracking-wide">New Restaurant Center</span>
...
// User menu with the role indicator: shows exactly what the app resolved from
// the CGOPS profile (or why it resolved nothing), so role/permission issues
        ...
        <div className="absolute right-0 top-full z-20 mt-1 w-72 rounded-lg border border-surface-line bg-surface p-3 text-sm shadow-lg">
          ...
          {!profile && (
            <p className="rounded-md bg-warning/10 px-2 py-1.5 text-xs text-warning">
              {profileError ?? 'No CGOPS profile resolved for this login.'}
            </p>
          )}
